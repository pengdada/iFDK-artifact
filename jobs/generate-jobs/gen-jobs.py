...
def generate_job(NP, NPP, ROWS, NX, NY, NZ, NU, NV, PROJS, SRC_DIR, DST_DIR, base):
	...
	with open(base, 'r') as text_base:
		...
		with open(job_name, "w") as text_file:
			...
			text_file.write('$MPIRUN -np $NP -ppn $NPP ./$mode/$process_name $NPP $ROWS $NX $NY $NZ $NU $NV $PROJS $SRC_DIR $DST_DIR $MAX_PROJ_DB\n')
	return job_name
...
if __name__ == "__main__":
	if len(sys.argv)  == 1:
		...
		if sys.argv[1] == "strong2k":
			strong_scability(2)
		elif sys.argv[1] == "strong4k":
			strong_scability(4)
		elif sys.argv[1] == "strong8k":
			strong_scability(8)
		elif sys.argv[1] == "weak2k":
			weak_scability(2)
		elif sys.argv[1] == "weak4k":
			weak_scability(4)
		elif sys.argv[1] == "weak8k":
			weak_scability(8)
